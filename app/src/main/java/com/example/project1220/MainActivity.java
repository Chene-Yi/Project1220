                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    if (response.code() == 200){
                        ...
                        for (int i = 0; i < items.length; i++) {
                            items[i] = "\n列車即將進入：" + data.result.results[i].Station + "\n列車行駛目的站" + data.result.results[i].Destination;
                        }

                        runOnUiThread(() -> {
                            new AlertDialog.Builder(MainActivity.this).setTitle("台北捷運到站站名").setItems(items, null).show();
                        });
                    } else if (!response.isSuccessful()) {
                        Log.e("Server error", response.code() + " " + response.message());
                    } else {
                        Log.e("another error", response.code() + " " + response.message());
                    }
                }
                @Override
